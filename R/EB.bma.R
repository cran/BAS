...
R2 = object$R2
  ...
  postmodelprob <- function(R2, p, n, g) {
...
if (is.null(g.0)) g.0 = sbest/(1 - sbest)
  ...
  post.prob = postmodelprob(object$R2,p, n, max(tau.0 - 1, 0))
...
while (abs(tau - tau.0) > tol | it < max.iterations) {
  ...
  post.prob = postmodelprob(object$R2,p, n, g)
  tau.0 = tau
...
logmarg  =  .5*((n -1 - p)*log(1 + g) - (n-1)*log( 1 + g*(1 - R2)))
logmarg[p == 0] = 0
postprobs = postmodelprob(object$R2,p, n, g)
